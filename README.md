...
End-to-end pipeline for processing whole genome sequencing (WGS) data into clinically actionable reports. Designed to run on consumer hardware (Unraid servers) using Docker containers with resource limits.
...
| 5 | [SV Annotation](docs/05-sv-annotation.md) | AnnotSV | `getwilds/annotsv:latest` | `diploidSV.vcf.gz` | `_sv_annotated.tsv` |
...
| 10 | [Telomere Length](docs/10-telomere-length.md) | TelomereHunter | `lgalarno/telomerehunter:latest` | `sorted.bam` | telomere content |
...
| 13 | [Functional Annotation](docs/13-functional-annotation.md) | VEP | `ensemblorg/ensembl-vep:release_112.0` | `.vcf.gz` | annotated VCF |
...
- Docker (amd64 host, or Unraid with Docker support)
...
export GENOMA_DIR=/mnt/user/Multimedia/Genoma
export SAMPLE=sergio
